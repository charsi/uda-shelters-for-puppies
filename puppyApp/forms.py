...
class NewPuppyForm(Form):
    ...
    weight = FloatField(
        'Weight', [validators.NumberRange(min=0.1, max=500, message='wth!')]
    )
    dob = StringField('Date of birth', [validators.Regexp(regex)])
    picture = StringField('Image URL', [validators.Regexp(regex)])
    description = StringField('More Info', [validators.Regexp(regex)])
